...
CREATE TABLE IF NOT EXISTS `ai_model_response` (
  ...
  `time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
  CONSTRAINT `fk_ai_model_response_system_prompt_hash`
    FOREIGN KEY (`system_prompt_hash`)
    REFERENCES `ai_model_content_hash` (`content_hash`)
    ON UPDATE CASCADE,
  CONSTRAINT `fk_ai_model_response_user_prompt_hash`
    FOREIGN KEY (`user_prompt_hash`)
    REFERENCES `ai_model_content_hash` (`content_hash`)
    ON UPDATE CASCADE,
)
ENGINE = InnoDB
